...
async def _fetch_youtube_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        logger.warning("YouTube transcript fetch failed for video %r", video_id, exc_info=True)
        return None
...
async def _apply_parsed_to_feed(
    ...
    incoming_guids = [g for g in incoming_guids if g]
    if incoming_guids:
        existing_guids: set[str] = {
            row[0]
            for row in db.query(Article.guid)
                .filter(Article.feed_id == feed.id, Article.guid.in_(incoming_guids))
                .all()
        }
    else:
...
async def _fetch_transcript(url: str) -> str | None:
    ...
    try:
        ...
        logger.warning("Transcript fetch failed for %r", url, exc_info=True)
        return None
