"""
Background scheduler — refreshes all active feeds every 30 minutes.
Publishes SSE events so connected clients get live new-article counts.
"""
import asyncio
import logging

from apscheduler.schedulers.asyncio import AsyncIOScheduler
from apscheduler.triggers.interval import IntervalTrigger

from ..database import SessionLocal
from ..models import Feed
from .feed_parser import refresh_feed
from .events import publish

logger = logging.getLogger(__name__)

scheduler = AsyncIOScheduler(timezone="UTC")


async def _refresh_all_feeds() -> None:
    db = SessionLocal()
    try:
        feeds = db.query(Feed).filter(Feed.is_active == True).all()
        logger.info("Scheduler: refreshing %d active feed(s)", len(feeds))
        for feed in feeds:
            try:
                new_count = await refresh_feed(feed, db)
                if new_count > 0:
                    await publish(
                        feed.user_id,
                        {"type": "new_articles", "feed_id": feed.id, "count": new_count},
                    )
            except Exception as exc:
                logger.warning("Failed to refresh feed %d (%s): %s", feed.id, feed.url, exc)
    finally:
        db.close()


def start() -> None:
    scheduler.add_job(
        _refresh_all_feeds,
        trigger=IntervalTrigger(minutes=30),
        id="refresh_all_feeds",
        replace_existing=True,
        max_instances=1,
    )
    scheduler.start()
    logger.info("Scheduler started — feeds refresh every 30 minutes")


def stop() -> None:
    scheduler.shutdown(wait=False)
    logger.info("Scheduler stopped")
